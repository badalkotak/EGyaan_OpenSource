                                            ...
                                            $.ajax({
                                                ...
                                                success:function(json)
                                                {
                                                    ...
                                                    if (status=="success") 
                                                    {
                                                        ...
                                                        var batch_dropdown = "<form action=add_timetable.php method=post><input type=hidden name=branch value="+branch+"><select name=batch  class='form-control select2' style='width: 100%;'><option value=-1 selected=selected>Select batch</option>"
                                                        for(var i=0;i<count;i++)
                                                        {
                                                            batch_dropdown = batch_dropdown + "<option value="+json.batch[i].id+">"+json.batch[i].name+"</option>";
                                                        }
                                                        batch_dropdown = batch_dropdown + "</select><button type='submit' class='btn btn-success' name='submit'><span class='fa fa-check'></span>Submit</button></form>";
